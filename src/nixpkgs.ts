import { err, ok, USER_AGENT, type Result } from "./utils";
...
export function match(provider: string): provider is "nixpkgs" {
  return provider === "nixpkgs";
}

export function parse(_provider: "nixpkgs", args: string[]): Result<Parameters, string> {
...
export async function handle(_request: Request, { status, variant }: Parameters): Promise<Response> {
